            ...
            for packet in self.packets:
                ...
                if seg_flag== 2: 
                    ...
                    if num>0:
                        ...
                        \n quiet time: {} \n Comp_S: {} \n Comp_K: {} \n Comp_M:{}'.format(
                                self.ical, UTC, live_time,quiet_time,compression_s,compression_k,compression_m)
                        ...
                        nbins= [32,12]
                        h=plt.hist2d(x,y,nbins,np.array([(0,32),(0,12)]),weights=z, cmin=1, cmap=plt.cm.jet)
                        ax.set_xticks(range(0,32,2))
                        ax.set_yticks(range(0,12,1))
                        title='Calibration run #{} at {}'.format(self.ical,UTC)
                    ...
                    for element in spectra_container:
                        ...
                        if ifig == 1 and last_detector!=det:
                            ...
                            fig=plt.figure(figsize=figsize)
                            fig.tight_layout()
                            fig.suptitle('Calibration run %d'%self.ical, fontsize=12, fontweight='bold')
                        ...
                        fig.tight_layout()
                        ax.step(np.linspace(0,num,num), spec, where='mid',label='Pixel {}'.format(pixel))
