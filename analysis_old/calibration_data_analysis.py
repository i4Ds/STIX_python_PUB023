...
def get_calibration_spectra(packet):
    ...
    spectra = [[int(it['raw'][0]) for it in item['children']] for item in cal
               if item['name'] == 'NIX00146']
...
def analysis(file_in, root_filename, pdf_filename, spec_log):
    alog = open(ana_log, 'a+')
    ...
    for i, d in enumerate(data):
        ...
        for row in results:
            if row['counts'] > 0:
                ...
                print('Detector %02d Pixel %02d: %0d event(s)' %
                      (row['detector'], row['pixel'], row['counts']))
                xlabel = 'ADC channel'
                ylabel = 'Counts'
                title = (
                    'Detector %d Pixel %d' % (row['detector'], row['pixel']))
                ...
                ipage += 1
                fr.cd()
                cc.Write("c_d_{}_{}_p_{}".format(ip, row['detector'],
                                                 row['pixel']))
...
def main():
    ...
    for f in os.listdir(raw_dir):
        if f.endswith(".dat"):
            ...
            parser.parse_file(raw_filename, l0_filename, [54124], 'binary',
                              'calibration run')
